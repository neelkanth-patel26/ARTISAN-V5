    ...
    <section className="relative w-full h-screen flex items-center justify-center overflow-hidden bg-gradient-to-b from-neutral-950 via-neutral-900 to-neutral-950">
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 text-center">
        ...
        >
          ...
          <motion.p 
            className="text-neutral-400 text-sm md:text-base leading-relaxed font-light max-w-2xl mx-auto mb-8 md:mb-12"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay: 0.4 }}
          >
            We connect collectors with exceptional artists worldwide. Every artwork is authenticated, insured during shipping, and comes with a certificate of authenticity.
          </motion.p>
            ...
            {[
              ...
              >
                <div className="relative mb-4">
                  <div className={`absolute inset-0 bg-gradient-to-br ${feature.color} opacity-0 group-hover:opacity-10 blur-2xl transition-opacity duration-500`} />
                  <feature.Icon className="relative w-10 h-10 md:w-12 md:h-12 mx-auto text-amber-600/70 group-hover:text-amber-600 transition-all duration-300 group-hover:scale-110" strokeWidth={1} />
                </div>
                <div className="text-base md:text-lg font-light text-white/90 mb-2" style={{ fontFamily: 'Oughter, serif' }}>{feature.title}</div>
                <div className="text-xs text-neutral-500 font-light">{feature.desc}</div>
              </motion.div>
            ))}
          </div>
        </motion.div>
