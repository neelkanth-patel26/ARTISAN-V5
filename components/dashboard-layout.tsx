...
export function DashboardLayout({ children, navItems, role }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-neutral-950 via-neutral-900 to-neutral-950 text-white overflow-x-hidden">
      ...
      {/* Mobile Header */}
      <header className="fixed left-0 right-0 top-0 z-50 flex h-auto min-h-[3.5rem] items-center justify-between border-b border-amber-600/20 bg-gradient-to-r from-neutral-950 to-neutral-900 px-4 pt-[max(8px,env(safe-area-inset-top))] [.native-mode_&]:pt-[max(48px,env(safe-area-inset-top))] pb-3 backdrop-blur-xl lg:hidden">
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <div
            className="fixed inset-0 z-40 mt-[max(8px,env(safe-area-inset-top))] [.native-mode_&]:mt-[max(48px,env(safe-area-inset-top))] bg-black/60 backdrop-blur-sm lg:hidden"
          ...
          >
            <motion.div
              ...
              className="absolute right-0 top-0 flex h-full w-64 flex-col border-l border-amber-600/20 bg-gradient-to-b from-neutral-950 to-neutral-900 pt-[max(8px,env(safe-area-inset-top))] [.native-mode_&]:pt-[max(48px,env(safe-area-inset-top))] pb-4"
      ...
      {/* Main Content */}
      <main className="relative min-h-screen pb-20 pt-[calc(3.5rem+max(8px,env(safe-area-inset-top,0px)))] [.native-mode_&]:pt-[calc(3.5rem+max(48px,env(safe-area-inset-top,0px)))] lg:pb-0 lg:pt-0 lg:pl-64 overflow-x-hidden">
